...
const bot = new Telegraf("1186843901:AAGxrXTZ6bkZtsDgAJTN4ln1YERzqmY3V2s"); // токен телеграмм бота 
const telegram = new Telegram(`1186843901:AAGxrXTZ6bkZtsDgAJTN4ln1YERzqmY3V2s`) // токен тг бота
...
const widgettoken = "6b28d493d39263cfafdb440bb3f87a822e532b9c04d2a07079136add2d7b40c5ba6503e42c6d46b3ebec6" //токен группы с доступом к виджетам
...
vk.setOptions({ 
	token: "69977967c79bb30c583fe5fe91b4f34cd99313dc9b9f8c4701c15314a38af2c7f12572a0cd8104e3ee307",  // токен группы (бота)
	apiMode: "parallel", // привед
	pollingGroupId: 190420120 // ид группы (бота)
});

const thisbotname = `Крутой бот` // имя вашего бота
const thisbotid = 190420120 // ид группы (бота)
const thisbotbeseda = `пока что не завели` // беседа вашего бота (ссылка, пример: https://vk.me/join/AJQ1d3ZZwxVAf3/E4sHsuyrI)
const thistelegabot = `KarieristttttBot` // ссылка на вашего телеграмм бота (пример: joraTELEGRAMbot)
// Настройки бота //
...
if(randm === 1){
	...
	for(let i in base.bs){
		if(base.bs[i].topenabled === true && base.bs[i].rank < 229){
			tops.push({
				id: i,
				idvk: base.bs[i].id,
...
if(context.isChat) {
	...
	if(chats.ids[context.chatId].active == 3) {
		let choose = getRandomInRange(0)
		chats.ids[context.chatId].active = 0
if(choose == 1) {
setTimeout(() => context.send({chat_id: context.chatId, message: ` :)`,
      keyboard: Keyboard.keyboard([
      [ 
            Keyboard.textButton({ 
            label: '📚 Помощь', 
...
setTimeout(() => context.send({chat_id: context.chatId, message: `:) `,
      ...
      [ 
            Keyboard.textButton({ 
            label: 'vk.com/bot_karieristtttt', 
            color: Keyboard.POSITIVE_COLOR,
            payload: {
            	"command": "help"
            }
...
updates.hear(/промо Карьерист|промокод Карьерист|промо Жорик|промокод Жора$/i,(context) => {
let platform = true
if(context.isChat) platform = true
let user = base.bs[base.id[context.senderId].id]
let nick = ``
...
let myclan = clan[base.bs[base.id[context.senderId].id].clan]
	...
	return context.send(`💎 Вы пригласили в игру: ${user.refs} чел.
...
updates.hear(/опыт топ$/i,(context) => {
let platform = false
if(context.isChat) platform = true
let user = base.bs[base.id[context.senderId].id]
let nick = ``
...
let myclan = clan[base.bs[base.id[context.senderId].id].clan]
    ...
    for (let i in base.bs){// перебор базы данных
        ...
        top.push({
            id: i,
            name: base.bs[i].nick,
             balance: base.bs[i].farms // создание массива
...
if (top.length < 10){
    for (let j = 0; j < top.length; j++){
    	...
    	b = b.replace("1", "2⃣")
    	b = b.replace("0", "1⃣")
        text += `${b} [id${base.bs[top[j].id].id}|${top[j].name}] — 👑${utils.rn(top[j].farms)}\n`
    }
} else {
    for (let j = 0; j < 10; j++){
    	j + 1
...
let myclan = clan[base.bs[base.id[context.senderId].id].clan]
	...
	if(user.balance < 100000000) return context.send(`${nick}, вам нужно 100.000.000$ для создания клана.`)
...
menu.urlButton('Karierist Bot', `vk.com/bot_karieristtttt`)
